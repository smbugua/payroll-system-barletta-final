...
<table class="table table-bordered  dataTables-example"  align="center" id="tableid" datapagesize="20"   >
    ...
    <tr>
        ...
        $bcode=$bank['bankcode'];
        $bbcode=$bank['branchcode'];
        $b=mysql_fetch_array(mysql_query("SELECT bank as b from banks where bcode='$bcode'"));
        $bb=mysql_fetch_array(mysql_query("SELECT bname as bb from bankbranch where bankCode='$bcode' and code='$bbcode'"));

        ?>
        <td><?php echo $b['b']?> </td>
        <td><?php echo $bb['bb']?> </td>
    ...
    <tfoot>
        ...
        <?php
        $rs=mysql_fetch_array(mysql_query("SELECT SUM(netpay) as saltot FROM payroll_tbl where payrollrun='$period'"));
        ?>
